...
export async function* streamEncryptedMedia(
  ...
  try {
    ...
    if (!data.ok) {
      throw new Error((data as any).error || 'Failed to retrieve encrypted media');
    }

    // Step 2: Unwrap media key (PQ-hybrid KEM with fallback to previous keys)
    console.log('[StreamDecoder] Unwrapping media key...');
    const dataObj = data as unknown as { wrappedKey: string | HybridCiphertext; chunk?: Uint8Array; byteLength?: number; ciphertext?: string };
    const wrappedKey: HybridCiphertext = typeof dataObj.wrappedKey === 'string'
      ? JSON.parse(dataObj.wrappedKey)
      : dataObj.wrappedKey;
    ...
    const ciphertextBytes = Buffer.from(dataObj.ciphertext || '', 'base64');
