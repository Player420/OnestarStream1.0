...
exports.detectSyncNeeded = detectSyncNeeded;
exports.validateSyncAlignment = validateSyncAlignment;
exports.generateSyncWarnings = generateSyncWarnings;
exports.getSyncRecommendation = getSyncRecommendation;
exports.getSyncHealthReport = getSyncHealthReport;
const keypairRotation_1 = require("./keypairRotation");
const keystoreExport_1 = require("./keystoreExport");
/**
 ...
 */
async function detectSyncNeeded() {
    const keystore = await (0, keypairRotation_1.loadEncryptedKeystore)();
    if (!keystore) {
        return {
            needsSync: false,
 ...
 */
async function validateSyncAlignment() {
    const keystore = await (0, keypairRotation_1.loadEncryptedKeystore)();
    ...
    const currentPublicKey = keystore.encryptedCurrentKeypair
        ? (0, keystoreExport_1.extractPublicKeyFromEncrypted)(keystore.encryptedCurrentKeypair)
        : '';
    if (!currentPublicKey) {
        return {
            aligned: false,
            ...
            if (entry.deviceId && entry.deviceName) {
                if (!deviceMap.has(entry.deviceId)) {
                    deviceMap.set(entry.deviceId, {
                        ...
                        firstSeen: new Date(entry.timestamp).getTime(),
                ...
                device.lastActivity = Math.max(device.lastActivity, new Date(entry.timestamp).getTime());
            }
        }
    }
    // Scan sync history
            ...
            if (sync.sourceDeviceId && sync.sourceDeviceName) {
                if (!deviceMap.has(sync.sourceDeviceId)) {
                    deviceMap.set(sync.sourceDeviceId, {
                        ...
                        firstSeen: sync.syncedAt,
                        lastActivity: sync.syncedAt,
                        rotationCount: 0,
                        syncCount: 0,
                    });
                }
                ...
                device.syncCount++;
                device.lastActivity = Math.max(device.lastActivity, sync.syncedAt);
            }
        }
    }
 ...
 */
async function generateSyncWarnings() {
    const warnings = [];
    const keystore = await (0, keypairRotation_1.loadEncryptedKeystore)();
    if (!keystore || keystore.version !== 'v4') {
        return warnings;
 ...
 */
async function getSyncRecommendation() {
    const keystore = await (0, keypairRotation_1.loadEncryptedKeystore)();
    if (!keystore || keystore.version !== 'v4') {
        return {
