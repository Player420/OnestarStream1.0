...
exports.createRotationAbortController = createRotationAbortController;
exports.acquireRotationLock = acquireRotationLock;
exports.releaseRotationLock = releaseRotationLock;
exports.isRotationInProgress = isRotationInProgress;
exports.forceReleaseRotationLock = forceReleaseRotationLock;
exports.migrateKeystoreV2ToV3 = migrateKeystoreV2ToV3;
exports.loadKeystoreV3 = loadKeystoreV3;
exports.needsRotation = needsRotation;
exports.getRotationStatus = getRotationStatus;
exports.getRotationHistory = getRotationHistory;
exports.rotateKeypair = rotateKeypair;
exports.unwrapMediaKeyWithFallback = unwrapMediaKeyWithFallback;
 ...
 */
function createRotationAbortController() {
    let aborted = false;
    const callbacks = [];
    return {
        abort() {
 ...
 */
function acquireRotationLock(userId) {
 ...
 * @param userId - User identifier
 */
function releaseRotationLock(userId) {
 ...
 */
function isRotationInProgress(userId) {
    if (userId) {
 ...
 */
function forceReleaseRotationLock(userId) {
    if (userId === '*') {
 ...
 */
function calculateNextRotation(intervalDays) {
    const now = new Date();
 ...
 */
function calculateKeyAge(createdAt) {
 ...
 */
async function migrateKeystoreV2ToV3(v2Keystore) {
    ...
    const keystorePath = (0, hybridKeypairStore_1.getKeystorePath)();
...
async function loadKeystoreV3() {
    ...
    if (keystore.version === 'v2') {
        ...
        await (0, hybridKeypairStore_1.saveKeystore)(v3Keystore); // TypeScript workaround
        return v3Keystore;
    }
    // Check if already v3
 ...
 */
function needsRotation(keystore) {
 ...
 */
function getRotationHistory(keystore) {
 ...
 */
async function rotateKeypair(currentKeypair, password, userId, reason = 'manual rotation', options) {
    ...
    try {
        ...
        const newKeypair = await (0, postQuantumCrypto_1.generateHybridKeypair)();
        ...
        if (options?.reWrapMedia !== false) {
            ...
            if (options?.reWrapAllMediaFn) {
                const result = await options.reWrapAllMediaFn(currentKeypair, newKeypair, options.abortController);
            ...
            if (mediaFailed > 0 && totalMedia > 0) {
                ...
                if (failureRate > rollbackThreshold) {
                    ...
                    if (keystoreBackup) {
                        await (0, hybridKeypairStore_1.saveKeystore)(keystoreBackup);
                    }
                    return {
                        success: false,
                        newKeyId: '',
        ...
        if (options?.encryptKeypairFn) {
            ...
            const encryptionKey = crypto.pbkdf2Sync(password, salt, 600000, 32, 'sha256');
        ...
        await (0, hybridKeypairStore_1.saveKeystore)(newKeystore); // TypeScript workaround
        // 7. Zeroize keys
 ...
 */
async function unwrapMediaKeyWithFallback(wrappedKey, currentKeypair, previousKeypairs) {
    ...
    const results = await Promise.allSettled(allKeypairs.map(async (keypair, index) => {
        try {
            const mediaKey = await (0, postQuantumCrypto_1.unwrapMediaKeyHybrid)(wrappedKey, keypair);
            return { success: true, mediaKey, index };
        }
        catch (error) {
