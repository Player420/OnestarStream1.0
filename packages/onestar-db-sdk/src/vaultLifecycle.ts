...
export interface VaultConfig {
  // Auto-lock settings
  idleTimeoutMs: number; // Default: 5 minutes (300,000ms)
  lockOnMinimize: boolean; // Default: false (user configurable)
  lockOnWindowBlur: boolean; // Default: false (user configurable)
  lockOnScreenLock: boolean; // Default: true (recommended)
  lockOnSleep: boolean; // Default: true (recommended)
  
  // Password policy
  minPasswordLength: number; // Default: 16 characters
  requireStrongPassword: boolean; // Default: true (entropy check)
  
  // Biometric settings
  biometricEnabled: boolean; // Default: true (if available)
  biometricFallbackToPassword: boolean; // Default: true
}
 ...
 */
export class VaultLifecycleManager extends EventEmitter {
  private state: VaultState = VaultState.LOCKED;
  private config: VaultConfig;
  private idleTimer: NodeJS.Timeout | null = null;
  private lastActivityTime: Date = new Date();
  private unlockStartTime: Date | null = null;
   ...
   */
  public updateConfig(config: Partial<VaultConfig>): void {
   ...
   */
  public async unlockWithPassword(password: string, userId?: string): Promise<UnlockResult> {
   ...
   */
  public lock(reason: string = 'Manual lock'): void {
   ...
   */
  private transitionState(newState: VaultState, reason: string): void {
  ...
  private async keystoreExists(): Promise<boolean> {
    try {
      const { keystoreExists } = await import('./hybridKeypairStore');
 ...
 */
let vaultLifecycleInstance: VaultLifecycleManager | null = null;
 ...
 */
export function getVaultLifecycle(config?: Partial<VaultConfig>): VaultLifecycleManager {
 ...
 * 
 * Convenience helper for preload scripts.
 * Returns true if VaultLifecycleManager state is UNLOCKED.
 * 
...
export function isVaultUnlocked(): boolean {
  ...
  return vaultLifecycleInstance.getState() === VaultState.UNLOCKED;
}
